...
boolean ecat_init(void)
{
    ...
    needlf = FALSE;
    inOP = FALSE;
	slave_EL3453 = (slave_EL3453_t *)IOmap;
	...
	while (run)
	{
		...
		if (inOP==TRUE)
		{
			...
			else
			{
				...
				rt_printf("Voltage: %f, Current: %f",slave_EL3453->TxPDO.chx[0].Voltage, slave_EL3453->TxPDO.chx[0].Current);
